...
$qu = "SELECT count(*) FROM user_ads WHERE job_owner=:job_owner";

$do = $dbfix->prepare($qu);	
$do->execute([':job_owner' => $job_owner]);
$listingCount = $do->fetch(PDO::FETCH_COLUMN);
if ($listingCount > 1) $lisTxt = "ads";
else $lisTxt = "ad";
	...
	$dbSelQu = "SELECT * FROM user_ads WHERE job_owner=:job_owner";
	$dbSel = $dbfix->prepare($dbSelQu);
	$dbSel->execute([':job_owner' => $job_owner]);
	$listMeat = $dbSel->fetchALL(PDO::FETCH_ASSOC);

	if ($listingCount < 1) echo "<p style=\"padding-right: 40px; font-weight: bold; font-size: 26px; text-shadow: 1px 1px 2px #111111;\">You don't yet have any ads.<br/>You can get started by filling out the form on the right.</p>";
	...
	foreach($listMeat as $listMeats)
	{
			...
			echo "<a href=\"/myaccount.php?activity=myads&amp;stateListings=1&amp;jobSelect=$jobID\" class=\"listing\" style=\"float:left\">
			$jobTitle<br/>Avail. Until: $jobDeadline<br/>
			Call-out Fee: &#163;$jobPrice
			</a>";
	}


echo "<br/><a href=\"/myaccount.php?activity=myads\" class=\"listing\" style=\"float:left; height: 28px; text-align: center; padding:4px; font-size: 20px; font-weight: bold;\">Create New Ad</a></div>";


	
	$dbSelQu = "SELECT * FROM user_ads WHERE id=:jobid";
	$dbSel = $dbfix->prepare($dbSelQu);
...
$jobDeadlineD  = $jobDeadlineDate[2];
	...
	if ($jobSelect < 1 && $activity == "myads")
	{
	echo "<div class=\"jobheader\" style=\"text-align: center;\">Create a new ad or select one to edit from the list.</div>";
	}
	else
	{
	$lastLoc = "ads";
	...
	echo "<div class=\"jobheader\" style=\"text-align: center;\">Edit ad: $jobTitle</div>";
	}
	...
	echo "<div class=\"form-style-2\"><form action=\"/account/myads.php\" method=\"post\">";


	if ($jobSelect > 0)
	{
	$titleGore = "Edit Ad Title";
	}
	else
	{
	$titleGore = "Create Ad Title";
	}	
		
	echo "<div class=\"jobsubheader\"><img class=\"icons\" src=\"/assets/images/icons/edit.png\"/> $titleGore</div>
	<div class=\"jobcontext\">Tip: Try to make your advertisement title compelling, but not 'salesy'</div>
	<p id=\"listingTitle\" name=\"listingTitle\" class=\"lineEdit\">$jobTitle</p>";	
	

	if ($jobSelect > 0)
	{
	$titleGore = "Edit Ad Description";
	}
	else
	{
	$titleGore = "Create Ad Description";
	}

		
	echo "<div class=\"jobsubheader\">
	<img class=\"icons\" src=\"/assets/images/icons/edit.png\"/> $titleGore</div>
	<div class=\"jobcontext\">Try to explain what you can and cannot fix.<br/>
	Beyond your call-out fee, try to give an indication of how much a job with you might cost, i.e. &quot;iPhone 7 Screen - &#163;90&quot;.<br/>
	Also give details of your availability and what happens if you cannot fix the device.</div>";
	
	echo "<div class=\"listingText\" id=\"listingText\" style=\"height: 400px;\" name=\"listingText\">$jobDesc</div>";
	
	
	
	echo "<div class=\"jobsubheader\"><img class=\"icons\" src=\"/assets/images/icons/edit.png\"/> Edit Ad Details</div>";
	

	echo "<div class=\"jobsubtitle\">Availability</div>
	<div class=\"jobcontext\">Availability is limited to keep things fresh; if you ever leave the site and do not return, your ad will drop off the listings. You can refresh this to a maximum of 30 days, as often as you like.</div>";
	...
	echo "<div class=\"jobsubtitle\">Call-Out Fee</div><div class=\"jobcontext\">This is how much charge for a call-out; this is not including the cost of the fix or any parts you may need.</div>
	...
	echo "<div class=\"jobsubheader\"><img class=\"icons\" src=\"/assets/images/icons/gallery.png\"/> Add Photos</div><div class=\"jobcontext\">You can include up to 9 photos per listing. To delete a photo, remove its thumbnail from the space below and submit your listing.<br/>
	</div>
	<div class=\"listingPhotos\" id=\"listingPhotos\" style=\"height: 240px;\">";
	
	$getPix = "SELECT * FROM user_gallery WHERE ad_id=:jobSelect";
	...
	echo "</div><div class=\"jobcontext\">Photos will appear larger on the published listing.<br/><br/></div>";
	...
	echo "<div class=\"gensubheader\" style=\"text-align: center; font-size: 24px;\">Once your listing has been created you will to add photos of your broken device.</div>";
	...
	echo "<button id=\"confirmation\" class=\"confirmation\" style=\"text-align: center; display: inline-block; width: 20%; border: 3px solid red;\" formaction=\"/account/myads.php\" >Cancel</button>";
	}
	else
	{
	...
	echo "<button id=\"confirmation\" class=\"confirmation\" style=\"text-align: center; display: inline-block; width: 16%; border: 3px solid red;\" formaction=\"/account/myads.php?jobID=$jobSelect&amp;stateListings=delist\" >Delete</button>";
	
		if ($jobStatus == 0)
		{
		echo "<button id=\"confirmation\" class=\"confirmation\" style=\"text-align: center; display: inline-block; width: 16%; border: 3px solid green;\" formaction=\"/account/myads.php?jobID=$jobSelect&amp;stateListings=liveness&amp;jobStatus=1\">Go Live</button>";
		}
		if ($jobStatus == 1)
		{
		echo "<button id=\"confirmation\" class=\"confirmation\" style=\"text-align: center; display: inline-block; width: 16%; border: 3px solid blue;\" formaction=\"/account/myads.php?jobID=$jobSelect&amp;stateListings=liveness&amp;jobStatus=0\">Take Offline</button>";
	...
	echo "</div><span style=\"padding: 12px;\">&nbsp;</span>
	</form></div></div>";
...
$countImages = "SELECT count(*) FROM user_gallery WHERE owner_id=:usrID AND ad_id=:jobSelect";
...
$grabImgFile = "SELECT * FROM user_gallery WHERE owner_id=:usrID AND ad_id=:jobSelect";
...
if (filter_var($emailAddress, FILTER_VALIDATE_EMAIL))
{
	...
	{ /// user email verified on account
		...
		{
		// no go
		$msg .= "This email address appears to be associated with another user. ";
		}
	}
	else
	{
...
{	// bad email
	$msg .= "The email address appears to be invalid. ";
}
...
//
	...
	$updatePrice = "UPDATE user_ads SET job_bounty=:dbPrice WHERE job_owner=:usrID AND id=:jobSelect";
	...
	{
	$updateTitle = "UPDATE user_ads SET job_title=:listingTitle WHERE job_owner=:usrID AND id=:jobSelect";
	...
	{
	$updateText = "UPDATE user_ads SET job_description=:listingText WHERE job_owner=:usrID AND id=:jobSelect";
	...
	$updateDeadline = "UPDATE user_ads SET job_deadline=:jobDeadlineDate WHERE job_owner=:usrID AND id=:jobSelect";
	...
	$msg = urlencode("Ad updated.");	
	$outGo = "/myaccount.php?msg=$msg&activity=myads";
...
if ($stateListings == "delist")
{
	if ($jobSelect != "")
	{
		...
		$usrID = $cooq["id"];
			...
			$stmt = $dbfix->prepare("DELETE FROM user_ads WHERE id=:jobSelect AND job_owner=:usrID");
			$stmt->execute([':jobSelect' => $jobSelect,':usrID' => $usrID,]);
	
	$msg = urlencode("Ad Deleted.");	
	$outGo = "/myaccount.php?msg=$msg&activity=myads";
	header("Location: $outGo");
	exit;
	}
	...
	{
	$msg = urlencode("Ad Potentially Deleted.");	
	$outGo = "/myaccount.php?msg=$msg&activity=myads";
	header("Location: $outGo");
	exit;	
	}
...
if ($stateListings == "liveness")
{
	...
	$updateListingStatus = "UPDATE user_ads SET job_status=:jobStatus WHERE job_owner=:usrID AND id=:jobSelect";
...
if ($stateListings == "newlisting")
{
	...
	if ($condition > 7)
	{
	$statement = $dbfix->prepare('INSERT INTO user_ads (job_title, job_description, job_owner, job_deadline, job_bounty) VALUES (?, ?, ?, ?, ?)');
	$statement->execute([$listingTitle, $listingText, $usrID, $jobDeadlineDate, $dbPrice]);
		
	
	$msg = urlencode("Ad created! Click on it from the list below to add some photos.");	
	$outGo = "/myaccount.php?msg=$msg&activity=myads";
	header("Location: $outGo");
	exit;
	}
	...
	{
	$msg = urlencode("Sorry, you already have the maximum 10 ads, please delete one and try again.");	
	$outGo = "/myaccount.php?msg=$msg&activity=myads";
	header("Location: $outGo");
	exit;		
	}
